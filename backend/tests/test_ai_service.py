...
class TestGeminiService(unittest.IsolatedAsyncioTestCase):
    """Test suite for Google Gemini AI analysis service with mocked SDK interactions."""

    def setUp(self):
        self.mock_api_key = "AIzaSyMockGeminiApiKey1234567890"
    ...
    async def test_case_1_successful_gemini_response(self):
        """Case 1: Successfully parse and validate structured Gemini response into DeveloperAnalysis."""
        with patch.dict(os.environ, {"GEMINI_API_KEY": self.mock_api_key}):
            ...
            with patch("app.services.ai_service.genai.Client") as mock_client_cls:
                ...
                mock_client.aio.models.generate_content = unittest.mock.AsyncMock(return_value=mock_response)
                ...
                # Verify client initialization and call kwargs
    ...
    async def test_case_2_malformed_json_response(self):
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": self.mock_api_key}):
            mock_response = MagicMock()
            mock_response.text = "This is totally not JSON at all {"
    ...
    async def test_case_4_http_401_from_gemini(self):
        """Case 4: Handle authentication failure from Gemini with clean HTTP 500."""
        from google.genai.errors import APIError

        with patch.dict(os.environ, {"GEMINI_API_KEY": self.mock_api_key}):
            with patch("app.services.ai_service.genai.Client") as mock_client_cls:
                mock_client = MagicMock()
                mock_client.aio.models.generate_content = unittest.mock.AsyncMock(
                    side_effect=APIError(401, {"error": {"message": "API_KEY_INVALID"}})
                )
                mock_client_cls.return_value = mock_client
                ...
                self.assertIn("Gemini authentication failed", ctx.exception.detail)
                self.assertNotIn(self.mock_api_key, ctx.exception.detail)

    async def test_case_5_http_429_from_gemini(self):
        """Case 5: Handle HTTP 429 Rate Limit / Quota Exceeded from Gemini with clean HTTP 429."""
        from google.genai.errors import APIError
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": self.mock_api_key}):
            with patch("app.services.ai_service.genai.Client") as mock_client_cls:
                mock_client = MagicMock()
                mock_client.aio.models.generate_content = unittest.mock.AsyncMock(
                    side_effect=APIError(429, {"error": {"message": "RESOURCE_EXHAUSTED"}})
                )
                mock_client_cls.return_value = mock_client
    ...
    async def test_case_6_http_500_from_gemini(self):
        """Case 6: Handle HTTP 500 from Gemini with clean HTTP 502."""
        from google.genai.errors import APIError
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": self.mock_api_key}):
            with patch("app.services.ai_service.genai.Client") as mock_client_cls:
                mock_client = MagicMock()
                mock_client.aio.models.generate_content = unittest.mock.AsyncMock(
                    side_effect=APIError(500, {"error": {"message": "Internal Server Error"}})
                )
                mock_client_cls.return_value = mock_client
                ...
                self.assertEqual(ctx.exception.status_code, 502)
                self.assertIn("Gemini AI service is temporarily unavailable", ctx.exception.detail)
                self.assertNotIn(self.mock_api_key, ctx.exception.detail)

    async def test_case_7_other_5xx_from_gemini(self):
        """Case 7: Handle other Gemini 5xx responses (e.g. 503) with clean HTTP 502."""
        from google.genai.errors import APIError
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": self.mock_api_key}):
            for status_code in [502, 503, 504]:
                with patch("app.services.ai_service.genai.Client") as mock_client_cls:
                    mock_client = MagicMock()
                    mock_client.aio.models.generate_content = unittest.mock.AsyncMock(
                        side_effect=APIError(status_code, {"error": {"message": "Upstream error"}})
                    )
                    mock_client_cls.return_value = mock_client
    ...
    async def test_case_8_gemini_request_timeout(self):
        """Case 8: Handle request timeouts to Gemini with clean HTTP 503."""
        with patch.dict(os.environ, {"GEMINI_API_KEY": self.mock_api_key}):
            with patch("app.services.ai_service.genai.Client") as mock_client_cls:
                mock_client = MagicMock()
                mock_client.aio.models.generate_content = unittest.mock.AsyncMock(
                ...
                self.assertIn("Couldn't reach Gemini AI service right now", ctx.exception.detail)
                self.assertNotIn(self.mock_api_key, ctx.exception.detail)

    async def test_case_9_missing_gemini_api_key(self):
        """Case 9: Handle missing or empty GEMINI_API_KEY with clean HTTP 500."""
...
# Backwards compatibility alias for test suite runners
TestOpenRouterService = TestGeminiService
